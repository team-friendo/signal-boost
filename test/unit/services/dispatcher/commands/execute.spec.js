...
import { commands, toggles } from '../../../../../app/dispatcher/commands/constants'
  ...
  describe('INVITE command', () => {
    ...
    describe('when vouching mode is on', () => {
      ...
      describe('when sender is not a member of channel', () => {
        const dispatchable = { sdMessage, channel: vouchingChannel, sender: randomPerson }
      ...
      describe('when sender is an admin', () => {
        ...
        describe('when all invitee numbers are valid (and unique)', () => {
          const dispatchable = { sdMessage, channel: vouchingChannel, sender: admin }
      ...
      describe('when sender is a subscriber on happy path', () => {
        const dispatchable = { sdMessage, channel: vouchingChannel, sender: subscriber }
        let res
        beforeEach(async () => {
          issueInviteStub.returns(Promise.resolve(true))
          countInvitesStub.returns(Promise.resolve(1))
