...
1. [Opening Issues](#issues)
   A. [Feature Requests](#features)
   A. [Bug Reports](#bugs)
1. [Submitting Merge Requests](#mrs)
1. [Reviewing Merge Requests](#review)
...
1. team assigns a reviewer if one has not already been assigned (for community-submitted MRs)
1. reviewer makes sure tests and lint rules are passing (manually or in CI)
1. reviewer pulls down branch, runs app, makes sure it behaves as the "Behavior" section in the corresponding issue describes (ie: that the MR meet's the issue's "acceptance criteria")
1. if feeling extra helpful, reviewer might poke around manually to make sure no regressions were introduced (at a base level: admins should be able to broadcast messages and new users should be able to subscribe and unsubscribe)
1. reviewer gives constructive code review (more on what that means to come), then:
   1. approves and merges
   1. asks for changes and merges once they have been made
1. if the work is functional but has clean up issues (difficult-to-maintain/hard-to-follow code, a useful refactor left unimplemented), the reviewer should consider:
   1. refraining from blocking shipping the MR over these issues, and instead
   1. opening a new issue -- the point of which is to clean up the MR under review -- and assign the MR author to implement it after the current MR is merged but before moving onto other work
1. if all of the above works, deploy!
   1. for now: @aguestuser will always do this manually
   1. long-term: @fdbk will investigate getting push-button deploy in gitlab (so that we can do manual testing first before automatically deploying)
