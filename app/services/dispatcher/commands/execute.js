...
const execute = async (executable, dispatchable) => {
  ...
  const result = await ({
    ...
    [commands.PRIVATE]: () => maybePrivateMessageAdmins(sock, channel, sender, payload, sdMessage),
    ...
    [commands.VOUCHING_ON]: () => maybeToggleSettingOn(channel, sender, toggles.VOUCHING),
    [commands.VOUCHING_OFF]: () => maybeToggleSettingOff(channel, sender, toggles.VOUCHING),
